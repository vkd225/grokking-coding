...
using namespace std;
int q_sum = 0;
vector<long long> solve (int Q, vector<int> type, vector<long long> X) {
   /*
   Q : Number of queries
   type : array denoting type of i'th query
   X :array denoting parameter of i'th query
   */
...
int main() {
    ...
    for(int t_i = 0; t_i < T; t_i++)
    {
        ...
        cin >> Q;
        q_sum+=Q;
        assert(Q <= int(1e5) && q_sum <= int(2e5));

        vector<int> type(Q);
        for(int i_type = 0; i_type < Q; i_type++)
        {
            cin >> type[i_type];
            assert(type[i_type] <= 3 && type[i_type] >= 1);
        }
        vector<long long> x(Q);
        for(int i_x = 0; i_x < Q; i_x++)
        {
            cin >> x[i_x];
            assert(x[i_x] <= int(1e9) && x[i_x] >= 0);
        }
